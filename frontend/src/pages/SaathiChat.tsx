...
import { Send, Sparkles, Heart, Wind, ShieldAlert, RefreshCw, MessageSquare, Play, Phone, WifiOff } from 'lucide-react';
...
export default function SaathiChat() {
  ...
  const sendMessage = async (text: string, currentMessages: ChatMessage[]) => {
    ...
    try {
      ...
      if (isNetworkErr) {
        setConnectionNotice("Unable to reach the MindScreen backend server at http://localhost:8000. Please ensure the backend is running.");
      }
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          content: isNetworkErr
            ? "I'm having a little trouble connecting to my thoughts right now (the MindScreen server on port 8000 might be resting). While we get that reconnected, let's take a slow, deep breath together. You don't have to carry this alone."
            : "I hear you, and I'm right here with you. Take a slow, deep breath with me. Things may feel heavy right now, but you don't have to carry them alone.",
          timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
          is_error: true,
          recommended_activity: {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      {/* Connection Notice banner if server down */}
      {connectionNotice && (
