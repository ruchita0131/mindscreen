...
(mean RMS loudness, energy variability, zero-crossing rate, spectral centroid,
spectral rolloff, and speaking ratio)---through decision-level late fusion.
...
sensitivity ablation across five constructed evaluation profiles;
(ii)~negation-aware disambiguation eliminating false alarms across a
9-statement negative/third-party adversarial test subset (0.0\% vs.\ 88.9\% naive
FPR) while retaining detection on 5/5 affirmative statements within a constructed
14-statement evaluation corpus; and (iii)~latency profiling showing 13.2~ms for
the local rule/heuristic fallback path and 512~ms for cloud-augmented inference.
...
Among the systems reviewed for this study, we did not identify a deployed
screening prototype that couples multimodal fusion with a formally specified
deterministic override to prevent high-risk false-negative dilution within a
lightweight cloud-hosted environment.
...
single high-priority screening tier ($c_3$, Table~\ref{tab:phq_map}).
Both bands trigger professional-referral and crisis-resource display in the
implemented system, motivating this operational grouping.
...
\begin{tabular}{@{}clll@{}}
\toprule
\textbf{Score $S$} & \textbf{PHQ-9 Band} & \textbf{Internal Tier ($\hat{y}$)}
...
with conventional PHQ-9 score intervals; they represent heuristic design
...
($\text{RMS}_\mu$), RMS energy variability ($\text{RMS}_\sigma$), zero-crossing
rate ($\text{ZCR}$), normalized spectral centroid ($S_C$), 85\% spectral rolloff
($S_R$), and speaking-to-silence ratio ($R_{\text{speak}}$).
These features have been investigated in the speech processing literature as
potential correlates of affective states and motor-speech changes~\cite{cummins2015};
in MindScreen, they are incorporated into a manually specified heuristic index
rather than a clinically trained diagnostic model.
...
piecewise linear interpolation across manually specified threshold intervals.
...
\begin{equation}
  p_k^{\text{cal}} = \frac{\exp(\log(\max(p_k^F, \epsilon))/T)}{\sum_{j=1}^4 \exp(\log(\max(p_j^F, \epsilon))/T)},
...
The base screening category and base score before escalation are:
\begin{equation}
  \hat{y} = \arg\max_k p_k^{\text{cal}},
  \qquad
  s_F = \max_k p_k^{\text{cal}}.
  \label{eq:prediction}
\end{equation}
...
(Tele-MANAS~\cite{telemanas2022}, KIRAN) whenever $C_F$ is active or as a
...
\STATE Compute score-softened vector $\mathbf{p}^{\text{cal}}$ via \eqref{eq:temperature} ($T=1.20$)
\STATE $\hat{y} \leftarrow \arg\max_k p_k^{\text{cal}}$;\;
  $s_F \leftarrow \max_k p_k^{\text{cal}}$;\;
...
TC-1: High score masked by positive text
  ...
  & Mild ($c_1$) & \textbf{Severe ($c_3$)} [$c_{\text{out}}=0.90$] \\
TC-2: Item~9 endorsed, low total score
  & $S=3$, $q_9=1$, neutral text
  & Minimal ($c_0$) & \textbf{Severe ($c_3$)} [$C_F=\text{T}$] \\
...
This demonstrates that unconstrained linear averaging is vulnerable to discordant
inputs regardless of the specific linear weighting, underscoring the safeguarding
utility of the deterministic HRE bound.
...
\midrule
P1: Concordant Severe & $0.68 \to c_3^*$ & $0.75 \to c_3^*$ & $0.56 \to c_3^*$ & $0.73 \to c_3^*$ & $0.70 \to c_3^*$ \\
P2: Concordant Minimal& $0.74$           & $0.80$           & $0.65$           & $0.76$           & $0.77$           \\
P3: Masked Affect     & $0.41 \to c_3^*$ & $0.60 \to c_3^*$ & $0.37 \to c_3^*$ & $0.49 \to c_3^*$ & $0.55 \to c_3^*$ \\
P4: Somatic Flatness  & $0.34$           & $0.57$           & $0.29$           & $0.45$           & $0.48$           \\
P5: Implicit Ideation & $0.36 \to c_3^*$ & $0.50 \to c_3^*$ & $0.35 \to c_3^*$ & $0.53 \to c_3^*$ & $0.46 \to c_3^*$ \\
\bottomrule
\multicolumn{6}{@{}l}{\footnotesize $^*$Escalated to internal tier $c_3$ by HRE deterministic bound ($c_{\text{out}} \leftarrow \max(0.90, s_F)$).}
\end{tabular}
\end{table}
...
Table~\ref{tab:negation}).
The scope-resolution filter achieved a 0.0\% false-positive rate
(0/9 false alarms, an 88.9\% absolute reduction) while retaining 100\%
detection on the 5/5 affirmative crisis assertions in this constructed set.
...
The local screening path (PHQ-9 + client-side acoustic scoring + keyword fallback)
executes within 25~ms and 25~MB of Python heap, ensuring operational resilience
if external APIs degrade.
Cloud-augmented inference (with the HuggingFace NLP API) adds approximately
500~ms of network round-trip time, yielding a 512~ms mean for the complete
screening pipeline.
...
by averaging, and a cloud-native deployment that fits within a free-tier 512~MB container.
