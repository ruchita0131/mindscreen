...
\label{alg:fusion}
\begin{algorithmic}[1]
\REQUIRE $\mathbf{q} \in \{0,1,2,3\}^9$, text $T$, audio features $\mathbf{f} \in [0,1]^6$
...
\STATE $\mathbf{p}^T \leftarrow \text{NLP}(T)$;\; $\mathbf{p}^A \leftarrow \text{Audio}(\mathbf{f})$
...
\centering
\resizebox{\columnwidth}{!}{%
\begin{tabular}{@{}p{2.6cm}p{2.2cm}cc@{}}
\toprule
\textbf{Scenario} & \textbf{Signal State}
  & \textbf{Fused Only} & \textbf{With HRE} \\
\midrule
TC-1: Masked high score
  & $S=22$, joy text, active audio
  & Mild ($c_1$) & \textbf{$c_3$} [$c_{\text{out}}\!=\!0.90, R_D$] \\
TC-2: Item~9 endorsed
  & $S=3, q_9=1$, neutral text
  & Minimal ($c_0$) & \textbf{$c_3$} [$c_{\text{out}}\!=\!0.90, C_F, R_D$] \\
TC-3: Crisis keyword
  & $S=2$, ``hopeless'' text
  & Mild ($c_1$) & \textbf{$c_3$} [$c_{\text{out}}\!=\!0.90, C_F, R_D$] \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{Evaluation Metric} & \textbf{Naive Substring} & \textbf{Scope-Resolved Filter} \\
\midrule
Affirmative Detection Rate      & 100.0\% (5/5) & \textbf{100.0\% (5/5)} \\
...
\bottomrule
\multicolumn{3}{@{}l}{\footnotesize All metrics calculated on the 14-statement constructed suite;} \\
\multicolumn{3}{@{}l}{\footnotesize not empirical estimates of clinical model performance.}
\end{tabular}%
}
\end{table}
