...
rather than a clinically trained diagnostic model. The index does not measure
depression directly, but provides a heuristic estimate of acoustic flatness and hesitancy.
...
TC-1: High score masked by positive text
  ...
  & Mild ($c_1$) & \textbf{High-priority ($c_3$)} [$c_{\text{out}}=0.90$] \\
TC-2: Item~9 endorsed, low total score
  & $S=3$, $q_9=1$, neutral text
  & Minimal ($c_0$) & \textbf{High-priority ($c_3$)} [$C_F=\text{T}$] \\
...
\midrule
P1: High Symptom Burden (Severe) & $0.68 \to c_3^*$ & $0.75 \to c_3^*$ & $0.56 \to c_3^*$ & $0.73 \to c_3^*$ & $0.70 \to c_3^*$ \\
...
These results should not be interpreted as empirical estimates of clinical
...
The reported cloud screening latency (512.0~ms mean) includes the local screening
operations (PHQ-9 mapping and acoustic scoring) and the Hugging Face inference API
request, including network round-trip time.
Saathi's Gemini request (390.0~ms mean) is measured separately during the interactive
dialogue stage and is not included in this assessment pipeline value.
