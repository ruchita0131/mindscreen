...
\begin{enumerate}
    \item \textbf{Severe Score Override}: If $S \ge 20$ (the validated psychometric cutoff for severe depressive symptoms established by Kroenke et al. \cite{kroenke2001}), the classification is deterministically forced to severe:
    \begin{equation}
...
The framework was evaluated using multimodal clinical session records from the Distress Analysis Interview Corpus (DAIC-WOZ) \cite{gratch2014} comprising 189 participant interaction sessions (107 train, 35 dev, 47 test), each paired with clinical PHQ-8/PHQ-9 assessments, interview transcripts, and synchronized vocal audio. The test cohort contained 14 sessions in the minimal tier, 11 mild, 10 moderate, and 12 severe (including 7 sessions with positive Item 9 self-harm endorsement).

Evaluation metrics include:
\begin{itemize}
    \item \textbf{Classification Performance}: Accuracy, Macro-Precision, Macro-Recall, and Macro-F1 across the four severity tiers.
    \item \textbf{Safety Robustness and Severe FNR}: Proportion of clinically severe or crisis cases ($S \ge 20$ or $q_9 > 0$) misclassified as minimal or mild.
    \item \textbf{Inference Latency}: Profiled on a cloud container with 512~MB RAM allocation and dual-core virtual CPUs.
\end{itemize}

\subsection{Multimodal Ablation and Performance Analysis}
Table \ref{tab:ablation} presents comparative results across individual modalities and fusion configurations. A grid-search sensitivity analysis over weighting combinations confirmed that the proposed weighting ($w_T=0.50, w_A=0.30, w_Q=0.20$) outperformed equal weighting ($0.33/0.33/0.33$, Macro-F1 = 0.831) and text-dominant weighting ($0.60/0.20/0.20$, Macro-F1 = 0.849), yielding the highest balanced Macro-F1 (0.864).
...
In the evaluated test set, the proposed CSO produced a severe false-negative rate of 0.0\%. This result demonstrates the deterministic behavior of the implemented rule-based override under the tested conditions; broader prospective clinical validation across diverse patient cohorts and unstructured expressions remains required.
